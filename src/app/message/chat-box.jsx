...
import { resizeBase64Image } from "@/lib/utils";
...
function ChatBox() {
  ...
  const handleCreateGroup = async (groupName) => {
    console.log("Group name:", groupName); // Check if groupName is correct
  ...
  const handleJoinGroup = async (groupId) => {
    console.log("Group ID:", groupId); // Debug: Check if this is a valid ObjectId
  ...
  const sendChatMessage = async (messageText) => {
    try {
      if (currentGroup && channel) {
        const resizedImage = await resizeBase64Image(user.photo, 100, 100);
